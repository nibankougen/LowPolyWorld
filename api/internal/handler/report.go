...
type reportWorldRequest struct {
	Reason string `json:"reason"`
	Detail string `json:"detail"`
}

// ReportWorld handles POST /api/v1/worlds/{worldID}/report.
func (h *Handler) ReportWorld(w http.ResponseWriter, r *http.Request) {
	reporterID := middleware.UserIDFromContext(r.Context())
	worldID := chi.URLParam(r, "worldID")

	var req reportWorldRequest
	...
	err := h.DB.QueryRow(r.Context(),
		`SELECT owner_user_id::text FROM worlds WHERE id = $1 AND is_public = TRUE`, worldID,
	).Scan(&ownerID)
	if err != nil {
	...
	if _, err := h.DB.Exec(r.Context(),
		...
		reporterID, ownerID, req.Reason, req.Detail,
	); err != nil {
		h.Logger.Error("report world: insert", "error", err)
