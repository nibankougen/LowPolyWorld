...
- ギミックアクション「効果音を鳴らす」の音量設定（0〜100%）は `WorldSFX` グループの相対値として乗算される
- 例: ユーザーの WorldSFX 音量 50%・ギミック音量 80% の場合 → 実効音量 40%
- ギミックアクション「BGM を切り替える」は `WorldSFX` グループの相対値として乗算される（音量設定はデフォルト BGM の設定値を引き継ぐ）

### 20.5 ワールド BGM システム

- 環境音と BGM を統合した内蔵サウンドライブラリから 1 トラックをループ再生
- ワールド作成者が「デフォルト BGM」をワールド定義 JSON（`worldBgm`）に保存
- 入場時にデフォルト BGM を自動再生（フェードイン・アウトあり）
- ギミックの「BGM を切り替える」アクションでトラックをオーバーライド可能。状態リセットでデフォルトへ復帰
...
| ワールド BGM 設定 | `soundId` が許可リスト内（`none` / `rain` / `ocean` / `wind` / `cave` / `darkFactory` / `bgmCheerful` / `bgmCalm` / `bgmSuspense`）/ `volume` が 0〜100 の整数 |
