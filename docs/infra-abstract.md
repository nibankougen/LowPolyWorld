...
直近1時間の最大PCUが **4,000 を超えた**場合、`pcu_threshold_warning` アラートを生成する（`docs/screens-and-modes.md` セクション 8.9 参照）+ `lowpolyworld-security-alerts` Pub/Sub → Discord（Vivox→LiveKit 移行作業の開始判断が必要）。
...
| 条件 | アラート種別 |
|---|---|
| 当月推計が月末に 4.5GB を超える見込み | `relay_usage_warning`（管理画面）+ `lowpolyworld-security-alerts` Pub/Sub → Discord（課金超過リスク・LiveKit 移行判断の材料） |
...
| 3 回失敗後 | 管理画面のシステムアラートに「Cloud Logging 転送失敗」アラートを生成（`docs/screens-and-modes.md` セクション 8.9 参照）+ `lowpolyworld-security-alerts` Pub/Sub → Discord（監査ログ外部保全の喪失）。フォールバックログは Cloud Logging 復旧後に手動インポートまたは再起動時の自動再送で取り込む |
| Cloud Logging 長期停止（1 時間以上） | Cloud Monitoring のアップタイムチェックが検知し、アラートポリシー経由でメール通知 |

**フォールバックログのディスク容量保護（2段階閾値）:**

Cloud Run のエフェメラルストレージ（デフォルト 512MB）消費を防ぐため、Go コードがファイルサイズを書き込みのたびに確認し以下の動作をとる。Discord 通知は `lowpolyworld-security-alerts` Pub/Sub → `discord-notifier` パイプライン経由（`infra-abstract.md §10` 参照）。

| 閾値 | 動作 | 通知先 |
|---|---|---|
| **40MB（警告）** | 書き込みは継続。`system_alerts` に `audit_fallback_log_warning` を記録 + Pub/Sub → Discord「フォールバックログが 40MB に達しました。Cloud Logging の状態を確認してください」 | 管理画面・Discord |
| **50MB（停止）** | 新規書き込みを停止。`system_alerts` に `audit_fallback_log_full` を記録 + Pub/Sub → Discord「フォールバックログが上限 50MB に達しました。外部監査ログの保全が停止しています。即時対応が必要です」 | 管理画面・Discord |

> 書き込み停止後も `admin_audit_logs` テーブルへの DB 書き込みは継続するため、改ざん防止の外部コピーは取れないが監査記録自体は失われない。Cloud Logging 復旧後に手動インポートで外部保全を再開する。
