...
- [ ] PlayerController（移動・ジャンプ・スプリント）
  - [ ] 物理コライダー設定（カプセル 直径0.4m / 高さ 1.45m）
  - [ ] タッチ判定コライダー設定（直径1.0m / 高さ2.0m）
  - [ ] Physics Layer 設定: `Player` レイヤーと `World` レイヤーを作成し、Layer Collision Matrix で `Player`-`Player` 間の衝突を無効化（アバター同士は重なることができる。`Player`-`World` 衝突は有効）
  - [ ] タッチ操作（画面下半分: 移動・ジャンプ / 上半分: カメラ回転）
    - [ ] 移動入力ロジック: スライド開始で移動開始。0.3 秒以内にスライド移動量が誤差範囲内に収まったらスプリント移行
    - [ ] マルチタッチ優先制御: 移動スライドは最初のタッチのみ有効。最初のタッチが離れるまで後続タッチの移動入力を無視
    - [ ] ジャンプ入力: 画面下半分のダブルタップでジャンプ（高さ 0.55）
    - [ ] 方向ジャンプ: 移動入力中に別指でダブルタップ → 移動方向へジャンプ
    - [ ] ダブルタップ→スライド移行: 2 回目タップからスライドに移行した場合、そのスライド方向へジャンプ
    - [ ] コントロールボタン（設定 ON 時）: ジャンプボタン・スプリントボタンを画面下部に表示。ボタン有効時はダブルタップジャンプ・スライド速度スプリントを無効化
    - [ ] 設定 19.5（コントロール設定）: ジャンプボタン/スプリントボタン表示 ON/OFF（PlayerPrefs 保存）
- [ ] 追従カメラ
- [ ] ブロブシャドウ（円形テクスチャを地面に投影）
- [ ] 共通アニメーションセット（待機・歩行・走り・ジャンプ）
- [ ] UI基盤（ナビゲーションバー: ワールド / アバター管理 / ワールド管理 / ショップ / 設定 の 5 タブ・デフォルト: ワールドタブ・画面遷移フロー）
- [ ] `SafeAreaFitter` コンポーネント実装（`Screen.safeArea` を参照してキャンバスルートの RectTransform を調整）・全キャンバスに適用
  - [ ] ワールドタブ（ホーム / フォロー中 / 新着順 / いいね の 4 タブ構成は Phase 5 でフル実装・Phase 1 はスタブ画面）
  - [ ] アバター管理タブ（Phase 2 でフル実装・Phase 1 はスタブ画面）
  - [ ] ワールド管理タブ（Phase 12 でフル実装・Phase 1 はスタブ画面）
  - [ ] ショップタブ（Phase 8 でフル実装・Phase 1 はスタブ画面）
  - [ ] 設定タブ（音量設定 3 スライダー・言語設定は Phase 5 で実装）
- [ ] インワールドHUD（右上: カメラ/ベル/☰ ボタン・左下: ジャンプボタン（設定ON時）・下中央: アクションボタン・右下: スプリントボタン（設定ON時））
- [ ] インワールドメニュー（☰タップで表示・右上×で閉じる・4タブ: ルーム/アバター/ワールド一覧/設定）
  - [ ] ルームタブ: ワールド詳細情報・ルーム情報（言語設定/上限/セッション残り時間）・再入室ボタン・ルームメンバー一覧
    - [ ] 言語設定変更（ルーム作成者のみ）
    - [ ] ルームメンバーパネル（プロフィール・フォロー/フレンド申請ボタン・その他（…）→非表示/通報）
    - [ ] 非表示メンバーをリスト下部に薄く表示
  - [ ] アバタータブ: 所持アバターサムネイルグリッド・サブタブ3種（スロット/ショップ購入/デフォルト）・アバター変更確認モーダル
  - [ ] ワールド一覧タブ: 入室時のワールド一覧をキャッシュして表示（滞在中は更新しない）
  - [ ] 設定タブ: 音量3スライダー（設定タブと共有）+ コントロール設定（セクション 19.5 と共有）
- [ ] ローカライゼーション基盤（`com.unity.localization` パッケージ設定・日本語/英語 String Table 初期作成・システム言語フォールバック実装）
- [ ] オーディオシステム
  - [ ] AudioMixer 設定（`WorldSFX` / `SystemSFX` グループ・exposed parameter 登録）
  - [ ] `AudioManager` 実装（各グループ音量制御・PlayerPrefs 永続化）
  - [ ] `AmbientSoundPlayer` 実装（soundId と volume を受け取り内蔵ライブラリからループ再生・フェードイン/アウト。ワールド設定 UI との接続は Phase 12 で行う）
  - [ ] システム音 SE の基本セット定義と再生（ボタン音など）
- [ ] `WorldSettingsLogic`（純粋 C#: 3 カテゴリの音量管理・クランプ・変更イベント通知）

### テスト（EditMode）
- [ ] `PlayerMovementLogic`: 移動状態遷移（待機→歩行→走り・ジャンプ可否判定）
- [ ] `CameraFollowLogic`: 追従オフセット計算・仰角クランプ
- [ ] `WorldSettingsLogic`: 初期値（1.0f）・クランプ（0f〜1f に収まる）・変更イベント発火・同値では発火しない
...
### Unityクライアント — 編集画面UI
- [ ] アバター編集画面（セクション 3.5）
  - [ ] 下部アイコンタブ（テクスチャ / アクセサリ）
  - [ ] アクセサリタブ: 4 サブタブ（利用中 / 保存・編集 / ショップ購入 / デフォルトプリセット）
  - [ ] アクセサリ選択状態: チェックマーク・設置ボーン選択フォーム・削除ボタン・ギズモ（移動/拡大縮小/回転）
  - [ ] アクセサリの D&D による取り付け
- [ ] アクセサリ編集画面（セクション 3.6）
  - [ ] 下部アイコンタブ（テクスチャ / 配置）
  - [ ] 配置タブ: 3D プレビュー拡大・つける場所選択フォーム・アバター切り替えボタン・ギズモ
- [ ] 編集画面共通UI（セクション 20）
  - [ ] 3D プレビュー: 1本指カメラ回転・2本指平行移動/ズーム・カメラ制限（対象が映っている状態を維持）
  - [ ] 下部タブ最小化（▽/△ボタン・イースイン・アウトスライドアニメーション）
  - [ ] Undo/Redo 50段階（定数 `PaintUndoMaxSteps` で管理・モバイルメモリに応じて調整可）（タブ直上左上 < > ボタン）

### テスト（EditMode）
- [ ] `AtlasLayout`: スロット割り当て（キャラ24枠・アクセサリ96枠）・UV 座標計算・スロット間パディング計算
- [ ] `AtlasLayout`: スロット満杯時の割り当て失敗・解放後の再利用
...
- [ ] プレイヤー位置・回転の同期（クライアント側線形補間）
  - [ ] 送信レート制御: 最大 20Hz・変化量が閾値（位置 > 0.01m または 回転 > 1°）を超えた場合のみ送信（Unity Relay 従量課金削減）
- [ ] アニメーション状態の同期（変更時のみ送信）
- [ ] アバター変更イベントの同期
- [ ] `WorldLoader` 実装（GLTF/GLBをAPIなしで静的パスから読み込む暫定版）
- [ ] `SessionTimeLimitLogic` 実装（セッション残り時間計算・警告イベント（残り10分・5分・1分）・制限時間到達イベント）
- [ ] `AfkDetectionLogic` 実装（操作なし経過時間計算・AFK判定・操作受信でタイマーリセット）
- [ ] ルームセッション制限 UI
  - [ ] 設定パネルへの残り時間表示（`残り 時:分:秒` 形式）
  - [ ] フラッシュメッセージ表示（残り10分・5分・1分のタイミング）
  - [ ] 制限時間到達ダイアログ（OK タップまたは数秒後に自動退室）
  - [ ] 放置自動退室（通常ユーザーのみ・10分無操作で即時退室）
- [ ] `ReconnectionLogic` 実装（指数バックオフ再接続: 1→2→4→8→16 秒・最大 5 回）
- [ ] ネットワーク切断 UI
  - [ ] 再接続中モーダル（「接続が切断されました。再接続しています…」＋「タイトルに戻る」ボタン・閉じ不可）
  - [ ] 接続失敗モーダル（「接続に失敗しました」＋「タイトルに戻る」ボタン・閉じ不可）
  - [ ] 再接続成功後: 通常入室と同じスポーンフローを再実行
- [ ] ルームオーナー移譲
  - [ ] オーナー退席時（切断・退室・放置退室）: サーバーが最長在室ユーザーに自動移譲・全クライアントへ通知
  - [ ] 新オーナーがギミックステートマスターの役割を引き継ぐ（言語・人数設定の変更権は元作成者のみ維持）
  - [ ] 元オーナー再入室時: 現オーナーから通常の後入室同期フローで状態を受け取る

> Unity Relay（本番用）はPhase 5以降で設定する。ローカル開発はDirect Connectionで行う。

### テスト（EditMode）
- [ ] `SessionTimeLimitLogic`: 残り時間計算の正確性・警告イベント発火タイミング（10分・5分・1分前）・制限時間到達イベント
- [ ] `SessionTimeLimitLogic`: 制限時間をゼロ以下にしたとき到達イベントが1回だけ発火すること
- [ ] `AfkDetectionLogic`: 10分無操作でAFK判定・操作受信でタイマーリセット・リセット後は再びカウント開始
- [ ] `ReconnectionLogic`: 指数バックオフ待機時間の計算（試行 n → 2^(n-1) 秒）・最大試行回数到達でFailureイベント発火・成功でSuccessイベント発火
...
#### コアエンジン
- [ ] レイヤーモデル実装（通常レイヤー最大16枚 + 色調補正レイヤー1枚/テクスチャ・別枠）
- [ ] UV レイヤー（最上段固定・表示オーバーレイのみ・枚数制限外）
- [ ] ベースレイヤー（最下段固定・2値マスク・枚数制限外）実装: ブラシ=不透明・消しゴム=透明・半透明不可
- [ ] レイヤーグループ実装（フォルダ状にネスト・グループ単位の表示切り替え）
- [ ] レイヤー合成エンジン（通常合成 → ベースレイヤーでマスク → PNG出力）
- [ ] 色調補正レイヤー処理（明度・彩度・コントラスト・色相シフト）
- [ ] 「下のレイヤーにマスク」（`mask_below`）対応
- [ ] レイヤー結合処理（選択レイヤーを1枚のラスターに統合）
- [ ] Undo/Redo履歴管理（50ステップ・定数 `PaintUndoMaxSteps` で管理・タブ単位でリセット）
- [ ] 透明ピクセル処理（保存時: α < 128 → α=0・RGB=(0,0,0)黒固定、α ≥ 128 → α=255・RGB 保持）
- [ ] テクスチャ範囲外データのクリーンアップ（編集終了時に自動実行）

#### ペイントツール処理
- [ ] ブラシツール: 円形アンチエイリアスなし（Nearest Neighbor・デフォルト）/ 円形アンチエイリアスあり（ソフトエッジ）。サイズ 1〜255px
- [ ] 消しゴムツール（α=0 に戻す）
- [ ] 塗りつぶしツール（フラッドフィル・許容値設定・他レイヤー参照オプション）
- [ ] 図形ツール（四角形 / 円 / 直線）
...
#### C ABI エクスポート
- [ ] `#[no_mangle]` + P/Invoke用インターフェース設計
- [ ] コマンド送受信API（キャンバスデータ・操作コマンド → 合成済みPNG返却）
...
- [ ] P/Invokeラッパークラス実装
- [ ] テクスチャ編集バー UI（色選択 / ブラシ / 消しゴム / 塗りつぶし / 図形 / 範囲選択 / 移動・拡大縮小 / レイヤー / その他）
- [ ] 色選択パネル UI（色相カラーサークル + 明度彩度四角形 + 透明度バー + RGBA入力 + 色履歴16色）
- [ ] ブラシサイズバー（画面左・1〜255px）
- [ ] 2Dキャンバスのピンチイン・アウトによる拡大縮小操作実装（最大約8倍・最小は全体表示）
- [ ] 3Dプレビュー（ペイント内容をリアルタイムにアバターへ反映して表示）
- [ ] レイヤーパネル UI
  - [ ] 新規レイヤー追加 / 効果レイヤー追加 / レイヤー取り込み / グループ作成 ボタン
  - [ ] レイヤー一覧: サムネイル・レイヤー名・表示非表示・その他アクション（削除/書き出し/透明度/マスク/ロック）
  - [ ] ドラッグ並び替え・グループへの出し入れ
  - [ ] UV レイヤー（最上段固定・色変更可）・ベースレイヤー（最下段固定）の表示
- [ ] その他メニュー UI（画像書き出し / テクスチャサイズ変更 / 簡単テクスチャ切り替えモーダル）
- [ ] 簡単テクスチャモード UI（スタブ・詳細仕様別途定義）
- [ ] Undo/Redo（テクスチャタブ内操作を `PaintUndoMaxSteps` 段階・< > ボタン）

### Unityクライアント — アバターペイントUI
- [ ] 256×256キャンバス表示・編集（1テクスチャ・区分なし）
- [ ] 保存処理
...
### テスト（EditMode）
- [ ] `LayerStack`: レイヤー追加・削除・複製・並び替え・上限チェック（16枚）
- [ ] `LayerStack`: 色調補正レイヤーの1枚制限・通常レイヤーとの独立カウント
- [ ] `LayerStack`: レイヤー結合（選択レイヤーを1枚に統合した結果の検証）
- [ ] `PaintCommandHistory`: Undo/Redo ステップ管理（最大50ステップ・境界値）
- [ ] `TransparentPixelProcessor`: α < 128 → α=0・RGB 黒固定 / α ≥ 128 → α=255 の変換
