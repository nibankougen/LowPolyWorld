...
- [x] インワールドメニュー（☰タップで表示・右上×で閉じる・4タブ: ルーム/アバター/ワールド一覧/設定）
  - [x] ルームタブ: ワールド詳細情報・ルーム情報（言語設定/上限/セッション残り時間）・再入室ボタン・ルームメンバー一覧（Phase 3 でフル実装）
    - [x] 言語設定変更ボタン（ルーム作成者のみ有効・非オーナーは無効スタイル）
    - [ ] ルームメンバーパネル（プロフィール・フォロー/フレンド申請ボタン・その他（…）→非表示/通報）— Phase 3
    - [x] 非表示メンバーをリスト下部に薄く表示（`RoomMemberListLogic.GetSortedMembers`）
...
- [x] 商品一覧 UI（アバター / アクセサリ / オブジェクト / スタンプ 各タブ）
  ...
  - [x] 名前検索・タグ検索
  - [x] オブジェクトタブのみ: テクスチャコスト別フィルター・コライダーサイズ別フィルター
  - [x] IDベースカーソルページネーション（スクロールで追加読み込み）
...
### `api/`（Go）
- [x] ユーザー情報取得エンドポイント（@name・称号・ショップリンク・フォロワー数・フォロー中数）
- [x] 違反報告送信エンドポイント（対象種別: ユーザー / ワールド / ショップ商品・通報理由8種・詳細テキスト・非表示同時実行オプション）
- [x] アバター直接利用（購入アバターをスロットなしで使用）のセッション管理
- [x] フレンド申請送信エンドポイント（上限チェック: 通常100人 / プレミアム1000人）
- [x] フレンド申請承認 / 拒否エンドポイント
- [x] フレンド申請キャンセルエンドポイント
- [x] フレンドリスト取得エンドポイント（承認済み / 申請中 / 申請受信）
- [x] フレンド解除エンドポイント
- [x] フレンドのみルーム参加制御（参加時にルーム作成者のフレンドリストをチェック）
- [x] フォローエンドポイント（`POST /users/{id}/follow`・`DELETE /users/{id}/follow`・レート制限付き）
- [x] フォロワーリスト / フォロー中リスト取得エンドポイント（`GET /users/{id}/followers`・`GET /users/{id}/following`）
- [x] フォロワー限定ルーム参加制御（参加時にルーム作成者のフォロワーリストをチェック）
- [x] 招待リンク発行エンドポイント（`POST /rooms/{id}/invite-link`・プレミアム権限チェック・既存リンクを無効化して新規発行）。使用回数上限はルームの `room.max_players` と同じ値を使用
- [x] 招待リンク情報取得エンドポイント（`GET /rooms/{id}/invite-link`・使用回数・有効期限）
- [x] 招待リンク検証・入室エンドポイント（`POST /invite/{token}/join`・有効期限/使用回数/ルーム状態を検証・初回入室ユーザーのみカウントアップ）
- [x] ルーム状態管理（OPEN / LOCKED / CLOSED）: 作成者退室時に LOCKED へ遷移・全員退室で CLOSED へ遷移
- [x] ユーザー非表示エンドポイント（`POST /me/hidden-users/{id}`・`DELETE /me/hidden-users/{id}`・レート制限付き）
- [x] ユーザー非表示リスト取得エンドポイント（`GET /me/hidden-users`）<!-- displayName・name も返すよう更新済み -->
- [x] ワールド非表示エンドポイント（`POST /me/hidden-worlds/{id}`・`DELETE /me/hidden-worlds/{id}`）
- [x] ワールド非表示リスト取得エンドポイント（`GET /me/hidden-worlds`）<!-- name も返すよう更新済み -->
- [x] 非表示時のフレンド関係自動解除処理
- [x] 非表示ユーザーからのフレンド申請をサーバー側でサイレント破棄する処理
- [x] アプリ内通知基盤（通知レコード作成・一覧取得・既読更新エンドポイント）
  - [x] フレンド申請受信時に通知レコードを生成
  - [x] フォロー中ユーザーのワールド公開時に通知レコードを生成（レート制限: 同一フォロー元1日1件）
  - [x] フォロー中ユーザーの商品販売開始時に通知レコードを生成（レート制限: 同一フォロー元1日1件）
- [x] iOS Universal Links / Android App Links 用ドメイン検証エンドポイント（`GET /.well-known/apple-app-site-association`・`GET /.well-known/assetlinks.json`）
...
- [x] バックグラウンド通話モード（iOS/Android・プレミアム限定）
  ...
  - [x] 3D描画停止処理
  - [x] プレミアム判定によるON/OFFの切り替え
- [x] フレンド管理画面（フレンド一覧タブ・申請中タブ・申請受信タブ・フレンドID検索）
- [x] フレンドがいるルーム一覧画面（`GET /me/friends/rooms` を使用・全ワールド横断・ワールド名/フレンドアイコン/参加人数表示・タップで参加）
- [x] フォロー管理画面（フォロー中タブ・フォロワータブ・フォロー解除ボタン）
- [x] 招待制ルーム作成フロー（プレミアム判定 → ルーム種別「招待制」選択 → 作成後に招待リンクパネルを表示）
- [x] 招待リンク管理 UI（コピーボタン・再発行ボタン・使用回数表示・有効期限残り時間表示）
- [x] Deep Link ハンドリング（iOS: Universal Links / Android: App Links）
  - [x] アプリ未起動時: 起動後にリンク処理を継続
  - [x] アプリ起動中: フォアグラウンドでリンクを受け取りルーム参加フローへ
- [x] 招待リンクエラー表示（有効期限切れ / 使用回数上限 / ルーム入室不可 / 無効リンク）
- [x] `HideListLogic` 実装
...
- [x] 通知設定 UI（設定画面内: 種別ごとの ON/OFF・PlayerPrefs 永続化）<!-- プッシュ通知権限リクエストは Phase 10+ -->
- [x] 撮影機能
  - [x] 撮影モード切り替え: 通常 HUD を非表示にして撮影専用 HUD（×/シャッター/スタンプ）に切り替え<!-- ロジック層: PhotoModeLogic 実装済み -->
  - [x] 撮影モード専用カメラ操作: 2 本指ピンチ/スライドでカメラ位置変更（ズーム・平行移動）。終了時に撮影前の位置に復元
  - [x] スタンプオーバーレイ配置: メニュー下から出す → メニュー外ドラッグで配置。ドラッグ/2本指回転/ピンチ縮小で操作<!-- ロジック層: StampOverlayLogic 実装済み -->
  - [x] ドラッグ中ゴミ箱アイコン表示 → ゴミ箱に重ねてスタンプ削除<!-- RemoveStamp で対応 -->
  - [x] スタンプ配置状態をルームセッション中メモリ保持（撮影モード再入時に復元）<!-- StampOverlayLogic は Clear() まで状態を保持 -->
  - [x] スタンプ選択メニュー: タブ構成（デフォルト/プレミアム/購入グループ）・プレミアムロック表示・最終選択タブのセッション保持
  - [x] スタンプ込み・UI なしのスクリーンショット撮影（`ScreenCapture.CaptureScreenshotAsTexture`・撮影時に HUD ボタンを一時非表示）
  - [x] 撮影と同時にカメラロールへ保存（iOS: `Photos API` / Android: `MediaStore API`）
  - [x] 撮影後左下サムネイル表示・タップで拡大・再タップまたは × で閉じる
  - [x] プレミアム限定スタンプのロック制御（非プレミアム: ロックアイコン + 購入誘導）
  - [x] 色変えスタンプ: サムネイル右下にカラーサークルインジケーターを表示
  - [x] 文字入れスタンプ 3 種類の実装（透明背景/角丸白文字/角丸黒文字）
  - [x] 文字入れスタンプ: 配置後テキスト入力モード移行（キーボード表示）・文字外タップで完了・スタンプ再タップで再編集
  - [x] 色変えスタンプ用カラーパレット UI（12 色横並び・横スクロール・スポイトボタン先頭）
    - 文字入れスタンプ: キーボード直上に表示（文字編集と同タイミング）
    - それ以外: 画面下部に表示
    - スタンプ外・パレット外タップで非表示・スタンプ再タップで再表示
  - [x] スポイトモード: 他UI非表示・円形スポイトカーソル・色サークル表示（白縁取り）・指を離して 0.4 秒で色確定・0.4 秒以内の再タップで再スポイト開始
  - [x] スポイト色サークルの貫通サンプリング（色サークル UI 自体を除外してスクリーンカラーを取得）
...
- [x] `ReportModalLogic`: 通報理由選択・詳細テキスト必須判定（理由8種）・送信可否・非表示チェックボックス制御
...
### Unityクライアント
- [x] BANユーザーのログイン拒否処理
- [x] 検疫中アバター（`moderation_status = 'pending'`）の扱い
  - [x] 本人のアバター一覧に「審査中」バッジ表示（アバター選択画面・インワールドメニュー）
  - [x] ルーム内で他プレイヤーが pending アバターを持つプレイヤーを見た場合、フォールバックアバター（システム既定）を表示
- [x] BAN済みアバター（`moderation_status = 'rejected'`）の非表示処理
  - [x] スロット表示（「非承認」バッジ・選択不可・アバター選択画面・インワールドメニュー）
  - [x] Atlas更新（拒否アバターのスロットをクリア）
- [x] ルーム参加時に 403 `user_restricted` を受信したとき制限エラー画面を表示（お問い合わせメールリンク: nibankougen@gmail.com）
- [x] 公認バッジ表示対応（`is_verified` フィールドを受け取り、表示名の右横に `icon_verified.png` を表示）
  - [x] ワールド内アバター頭上の World Space Canvas 名前タグ（名前タグシステム未実装）
  - [x] ユーザー情報パネル（セクション 2.4）
  - [x] アカウント情報画面（セクション 22.1）（画面未実装）
  - [x] フレンド一覧・フォロー/フォロワー一覧など表示名が出る箇所
...
- [x] ワールドエディタ UI（セクション 11.7）
  ...
  - [ ] オブジェクト数上限チェック（合計 400）— `ObjectPlacementStore`（MaxObjects=400）で実装済み・UI 連携は未
  - [~] **オブジェクトタブ**（セクション 11.7.3）— UI 縦スライス実装済み（`ObjectTabController` + `ObjectEditSceneController`・プレースホルダ Cube 描画・Play で動作確認可。メニュー「オブジェクトタブ統合プレビューを生成」）。グループ UI・複数選択・D&D・実 GLB・特殊オブジェクトは未
    - [x] タブ固定ヘッダー: オブジェクト数合計/上限（コストは共有表示）
    - [x] 利用中: サムネイル（色スウォッチ）グリッド・テクスチャコスト表示
    - [ ] 利用中: D&D 並び替え（描画順対応）・複製は直後に挿入（複製=`ObjectPlacementStore.Duplicate` 実装済み・D&D 未）
    - [ ] グループ: 作成（選択中アイテムからグループ化）・名前設定（1〜20文字・連番デフォルト）・▶/▼ 開閉トグル・D&D で出し入れ — グループ操作ロジック（作成/改名/削除=子繰り上げ/再親付け/同一階層判定）は `ObjectPlacementStore` 実装済み・UI 未
    - [ ] グループ: 最大 4 段ネスト・合計 32 個上限 — `ObjectPlacementStore`（MaxNestDepth=4 / MaxGroups=32・深さ/循環検証）で実装済み
    - [ ] グループ操作: 移動/拡大縮小/回転を各オブジェクトに個別適用・いずれか不可の場合キャンセル＋フラッシュメッセージ
    - [ ] 複数選択: 同一階層レベルのみ・異階層選択時エラー・複数選択時は移動のみ（拡大縮小/回転不可）
    - [ ] 保存・編集サブタブ: 編集オブジェクトスロット + 保存バリアント一覧（スタブ表示のみ）
    - [~] 所有サブタブ: デフォルト・購入済み・特殊（プレースホルダ種別パレットのみ・購入済み/特殊は未）
    - [x] 利用中以外タップ → 原点（0,0,0）に配置・選択状態（D&D 配置は未）
    - [x] 選択状態: 複製ボタン・削除ボタン（テクスチャ編集ボタンは未・Phase 4 依存）
  - [x] ギズモ操作: 移動（ドラッグ→0.5m スナップ）・回転（タップ→Y軸45°）・拡大縮小（W/D/H ステッパー 0.25m・スケールロック判定）— `ObjectGizmoLogic` 経由
  - [~] 重複判定: スポーン・ポータルのコライダー重複を検出 → 警告表示・プレイ/公開ブロック — ロジック `SpecialObjectOverlap`（1×1.5×1 AABB・特殊同士 + `IWorldOccupancyQuery` 経由で地形/オブジェクト判定）+ `WorldPublishValidator.SpawnPortalOverlap` 実装済み。`IWorldOccupancyQuery` の占有ロジック（`TerrainOccupancyQuery` 地形ボクセル保守的占有・`WorldObjectOccupancyQuery` 配置オブジェクトコライダー・`CompositeOccupancyQuery` 合成 = `WorldOccupancy.cs`・純粋C#・テスト済み）実装済み。MonoBehaviour 配線（TerrainRenderer/配置シーンへの接続・worldOrigin 供給）と警告フラッシュ UI 配線は未
...
- [ ] **ギミック表現力拡張（ストーリー / セリフ / シーケンス）**（world-creation.md 9.7b / 9.13・ノードベース UI は採用せず ECA + 新プリミティブで実現）
  - [~] 会話システム（9.13）
    ...
    - [~] 会話エディタ UI（11.7.4b・会話一覧 / 行 / 選択肢 / 多言語入力）— `ConversationLibraryController` + `ConversationEditorController` 実装済み（人間の見た目確認待ち）。到達時 / 選択時の変数変更（onReach / effect）UI も実装（種別なし/ワールド変数/プレイヤー変数・対象・演算・値・定義済み変数ドロップダウン）。残: 言語別「詳細」入力・行 D&D
  ...
  - [ ] 待機アクション（9.7b・シーケンス）
    ...
    - [ ] ルール編集 UI に待機アクションの秒数入力
  - [ ] サブルーチン（9.5「呼び出された」イベント + 9.8「サブルーチンを呼ぶ」アクション）
    - [ ] 入力イベント `called` + アクション `callSubroutine`（サブルーチン ID）の変換・バリデーション
    - [ ] エンジンでの呼び出し実行（呼び出し元の入力/相手プレイヤー引き継ぎ・連鎖イベントとしてループカウント）+ テスト
  - [ ] `GimmickTypeCatalog` / `GimmickRuleEditLogic` に新種別（startConversation / wait / callSubroutine / called）を追加
- [x] 設定タブ（ワールド設定パネル UI）— `WorldEditor.uxml/uss` + `WorldEditorController`
  ...
  - [x] ワールド名・タグ（最大 5 個）・背景設定・環境カラー（カラーピッカー・V ≥ 0.25 制限）・環境音・人数上限・公開/非公開
  - [x] フォグ設定 UI（ON/OFF トグル・カラーピッカー・開始/終了距離スライダー）
  - [x] スクリーンエフェクト設定 UI（種類ドロップダウン・強度スライダー）
  - [x] 「新しいバージョンを公開」ボタン（詳細は 11.7.6 公開フロー参照）— `WorldPublishValidator` + UI エラー表示
  - [ ] バックアップ保存（プレミアムのみ・5 枠・上書き確認ダイアログ・異常ファイルサイズのみバリデーション）
  - [ ] バックアップ復元（プレミアムのみ・復元確認ダイアログ）
- [x] ギミック実行エンジン（入力イベント検知・条件評価・アクション実行）
  - [x] 全入力イベント実装（ルーム開始/人数変動/オブジェクト接触/タップ/エリア/タイマー/アクションボタン/プレイヤー接触/リスポーン/ポータル利用）
  - [x] 相手プレイヤー確定（PlayerTouchPlayer イベント・距離/視線/重なり条件での動的確定）
  - [x] 全条件実装（ワールドステート/プレイヤーステート/人数/番号/インベントリ※IInventoryQuery 経由/物理判定3種）
  - [x] 比較演算に mod 追加（CompareOp.ModEquals）
  - [x] 全アクション値参照（固定値/ワールドステート/プレイヤーステート/全プレイヤー合計/範囲乱数）
...
- [ ] 地形システム
  ...
  - [ ] **Unity クライアント**
    ...
    - [x] 動的メッシュ生成（チャンク単位・隣接判定による面カリング）— `TerrainMeshBuilder`
      - [x] cube / ramp / diag 各形状の面生成（North 基準形状の XZ 回転で導出）
      - [x] 隣接判定ルール実装（セクション 15.12 の形状別テーブル準拠）— `TerrainNeighborRules`
      - [x] チャンク境界の隣チャンク参照 — `ITerrainVoxelSampler` / `TerrainStoreSampler`（未ロード時「地形あり」はストリーミングサンプラー実装時）
      - [x] 下面メッシュも露出面のみ常時生成（ワールド下端は「地形あり」扱いで地表の底面は生成しない — 15.14）
    ...
    - [ ] **地形タブ UI**（セクション 11.7.2）
      - [x] 編集ロジック（ブラシ/消しゴム/図形=四角形/タイプ変更サイクル/範囲選択=矩形・塗りつぶし/移動/コピー&ペースト/Dirty チャンク追跡 — 純粋 C#）— `TerrainEditLogic`
...
**目標**: 撮影画像を 48 時間限定で共有できる状態（仕様: `docs/screens-and-modes.md` セクション 23 / `docs/api-abstract.md` セクション 16）
※ Phase 9（フォロー/フレンド・撮影モード・通知/非表示基盤）・Phase 11（違反報告）が完成していることが前提

### `api/`（Go）
- [ ] `stories` / `story_report_snapshots` テーブル（マイグレーション）
- [ ] `POST /stories`: 画像アップロード（9:16 正規化・1080×1920 以下）+ 公開範囲 + 撮影ワールド ID。同時有効 10 件制限（409）・ユーザー制限中は 403
- [ ] `GET /stories/feed`: フォロー中ユーザーの有効ストーリー（公開範囲・非表示/ブロック考慮・ユーザー単位グルーピング・最新投稿時刻降順）
- [ ] `GET /users/{id}/stories`: 公開範囲を考慮した一覧（friends はフレンドのみ）
- [ ] `DELETE /stories/{id}`: 本人のみ・即時物理削除
- [ ] `story_likes` テーブル + `POST/DELETE /stories/{id}/like`（冪等・公開範囲検証・自分のストーリーは 403）
- [ ] レスポンスに `liked`（全員）/ `likeCount`（投稿者本人のみ）を付与
- [ ] 画像配信: 短寿命署名付き URL（10 分）発行。content-addressed CDN は使わない（アクセス制御・失効残留対策）
- [ ] 失効バッチ（1 時間ごと）: 期限切れレコード + R2 オブジェクトの物理削除
- [ ] 違反報告 API に `target_type=story` 追加 + 通報受理時の画像スナップショット保存
- [ ] スナップショット削除バッチ（審査完了 + 90 日で物理削除）
- [ ] アカウント削除時の有効ストーリー即時削除（スナップショットは保持）

### 管理画面
- [ ] 違反報告一覧にストーリー通報を統合 + スナップショット画像表示（8.4）
- [ ] 違反確定時の該当ストーリー即時削除操作

### Unity
- [ ] 撮影プレビュー拡大表示に「ストーリーに投稿」ボタン + 公開範囲選択（テストプレイ中は非表示）
- [ ] ホームタブ最上部のストーリーアイコン行（自分が左端・最新投稿順・未読カラーリング/既読グレーリング・端末ローカル既読管理）
- [ ] ストーリービューア（5 秒自動送り・タップ送り/戻し・長押し一時停止・プログレスバー・ワールドリンク → 推奨公開ルーム参加・… メニュー: 通報/削除）
- [ ] ビューア右下のハートボタン（いいねトグル・自分のストーリーには非表示・自分のビューアには「♥ N」のいいね数表示）
- [ ] アカウント情報パネルのストーリーリング表示・閲覧導線
- [ ] ロジッククラス + EditMode テスト（表示順・既読管理・自動送りタイマー・公開範囲/非表示フィルター・ワールドリンク有効判定）

### テスト（Go）
- [ ] 48 時間失効: バッチでレコード・オブジェクトとも物理削除されること
- [ ] フレンドのみ公開: フレンド以外の取得で除外されること
- [ ] 同時有効 10 件超過の投稿が 409 で拒否されること
- [ ] 通報スナップショット: 元ストーリーの失効・削除後も管理 API から画像を取得できること
- [ ] アカウント削除: 有効ストーリーが即時削除され、スナップショットは残ること
- [ ] いいね: `likeCount` が投稿者本人以外のレスポンスに含まれないこと
- [ ] いいね: フレンドのみ公開のストーリーに非フレンドがいいねできないこと・自分のストーリーへのいいねが 403 になること
...
Phase 9 → Phase 13
Phase 9 + Phase 11 → Phase 14
Phase 9 + Phase 11 → Phase 15（ストーリー）
```
