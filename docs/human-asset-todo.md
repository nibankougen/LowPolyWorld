...
> Aseprite / GIMP 等で 256 × 256 のドット絵を描き、8 × 8 グリッド（各 32 px）に面ごとのパターンを配置する。バリアントが多いほど自然に見えるが、最低限バリアント 0 の 4 列だけ描いても動作確認は可能。

**仕様（固定地形テクスチャ）:**
- サイズ: **32 × 256** px
- 形式: PNG（アルファチャンネルあり可。透明ピクセルの扱いはランダム地形テクスチャと同様）
- レイアウト: **1 × 8** グリッド（各セル 32 × 32 px、下から順）
  - 行 0（最下行）: 下面
  - 行 1: 坂側面
  - 行 2: 坂側面下端
  - 行 3: 側面上端下端
...
> Aseprite / GIMP 等で 32 × 256 のドット絵を描き、32 px ごとの行に面ごとのパターンを配置する。バリアントがない分、1 行あたり 32 × 32 の 1 パターンのみ描けばよい。
