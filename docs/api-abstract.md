...
POST /api/v1/me/parental-consent/send
  ...
  処理:
    ...
    2. 指定メールアドレスへ確認メールを送信（メールアドレスは送信後即時破棄）
    3. jti を Redis に保存（TTL 72 時間。アドレス変更時の旧リンク無効化に使用）
...
| ジョブ | 実行タイミング | 処理 |
|---|---|---|
| `parental_consent_reminder` | アカウント作成から 7 日後（同意未完了の場合のみ） | リマインドメールを送信（「あと 7 日で登録が取り消されます」）。送信先メールアドレスは最後に `send` エンドポイントに渡されたものを Redis から取得（TTL 14 日で別途保持）。取得できない場合はスキップ |
...
リマインド送信のため、最後に使用されたメールアドレスを Redis に 14 日間保持する（`parental_consent_email:{user_id}` キー）。同意完了時・14 日タイムアウト時に削除する。DB には保存しない。
...
同一ファイルへの削除が 3 回以上連続して失敗した場合（リトライ3回目時点）、管理画面のシステムアラート（`docs/screens-and-modes.md` セクション 8.9）に「ストレージ削除失敗」アラートを生成する。管理者はアラート画面から孤立ファイルの一覧を確認し、手動削除を実行できる。
...
3. Optimizer worker（asynq worker）→ キューからジョブを取得・処理:
                    ...
                    e. 最適化済み VRM を /avatars/{hash}.vrm に保存
...
| 失敗時処理 | バッチ全体が失敗した場合、Cloud Scheduler の自動リトライ（最大 3 回・30 分間隔）で再実行。3 回失敗後は管理画面のシステムアラートに「匿名化バッチ失敗」を生成 |
...
再取得に失敗（タイムアウト / 5xx）した場合:
  ...
  → 3 回失敗後: 管理画面にシステムアラートを生成（「JWKS 取得失敗: {provider}」）
```
...
| ケース | ログレベル | アラート |
|---|---|---|
| 正規の `kid` で署名検証失敗 | **ERROR**（鍵偽造・トークン改ざんの疑い） | 5 分間に 10 件以上で管理画面にアラート生成 |
